...

{
  ...
  # If the pull conflicts (e.g. manifest.json diverged after a failed push),
  # hard-reset to remote — everything local is regenerated by the pull anyway,
  # so remote-wins is always safe here and keeps the job self-healing.
  if ! git -c "$CRED" pull --rebase --autostash "$HTTPS" main; then
    echo "WARN: pull failed — hard-resetting to remote"
    git rebase --abort 2>/dev/null; git merge --abort 2>/dev/null
    git -c "$CRED" fetch "$HTTPS" main && git reset --hard FETCH_HEAD
  fi
